...
export function BeachList({ beaches, selectedTimeIndex, sortBy, searchQuery }: BeachListProps) {
  const filteredAndSortedBeaches = useMemo(() => {
    ...
    // Apply search filter
    if (searchQuery) {
      filtered = beaches.filter((beach) => beach.name.toLowerCase().includes(searchQuery.toLowerCase()))
    }
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
        <div className="text-muted-foreground">
          {searchQuery ? `No beaches found matching "${searchQuery}"` : "No beaches available"}
        </div>
      </motion.div>
    )
  }
